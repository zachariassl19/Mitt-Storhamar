...
function HomePage({
  ...
  const upcoming = games
    .filter((game) => new Date(game.startsAt).getTime() > new Date(nextGame.startsAt).getTime())
    .slice(0, 4)
    ...
    <>
      ...
      <section className="card season-card">
        ...
        <div className="stat-grid">
          <div><strong>{ehlCount}</strong><span>EHL-kamper</span></div>
          <div><strong>{chlCount}</strong><span>CHL-kamper</span></div>
          <div><strong>{games.length}</strong><span>Kamper totalt</span></div>
        </div>
...
function GamesPage({
  ...
  return (
    <section className="page-section">
      ...
      {future.length > 0 && (
        <>
          <div className="games-subheading"><span>KOMMENDE</span><strong>{future.length}</strong></div>
      ...
      {past.length > 0 && (
        <>
          <div className="games-subheading past-heading"><span>TIDLIGERE KAMPER</span><strong>{past.length}</strong></div>
          <div className="game-card-list">
...
function GameRow({ game, plan, onOpen }: { game: Game; plan: AttendancePlan; onOpen: () => void }) {
  ...
  return (
    <button className="game-row game-row-with-logos" onClick={onOpen} type="button">
      ...
      <div className="logo-pair" aria-hidden="true">
      ...
      <div className="game-row-main">
        <div className="game-meta">
          ...
          <span>{isHome(game) ? 'HJEMME' : 'BORTE'}</span>
          {game.special && <span className="special-tag">{game.special}</span>}
          {!result && plan !== 'unset' && <span className={`tiny-plan ${plan}`}>{planLabel(plan)}</span>}
        </div>
        <strong>{game.homeTeam} – {game.awayTeam}</strong>
        <small>{game.arena} · {result ? 'Ferdig' : timeText(game)}</small>
      </div>
      {result ? (
        <div className="game-result">
          <strong>{game.homeScore}–{game.awayScore}</strong>
          {game.decisionType && game.decisionType !== 'REG' && <span>{decisionLabel(game).trim()}</span>}
        </div>
      ) : (
...
function TeamMark({ name, primary = false }: { name: string; primary?: boolean }) {
  return (
    <div className={`team-mark ${primary ? 'primary' : ''}`}>
      <TeamLogo team={name} hero />
